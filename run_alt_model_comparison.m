...
% networkFlag = 15; lam = 0.1; gam = 0.05; nTrials = 100; run_alt_model_comparison(networkFlag, lam, gam, nTrials); 
% networkFlag = 23; lam = 0.1; gam = 0.05; nTrials = 100; run_alt_model_comparison(networkFlag, lam, gam, nTrials); 
% networkFlag = 46; lam = 0.1; gam = 0.05; nTrials = 100; run_alt_model_comparison(networkFlag, lam, gam, nTrials); 
% networkFlag = 56; lam = 0.1; gam = 0.05; nTrials = 100; run_alt_model_comparison(networkFlag, lam, gam, nTrials); 
% 
% lam = 0.1; gam = 0.05; nTrials = 100; for networkFlag = [15, 23, 46, -4, -3, 0]; run_alt_model_comparison(networkFlag, lam, gam, nTrials); close all; end
% lam = 0.05; gam = 0.2; nTrials = 100; for networkFlag = [15, 23, 46, -4, -3, 0]; run_alt_model_comparison(networkFlag, lam, gam, nTrials); close all; end
% 
% 
% Moore et al. (2024), "Network spreading from network dimension"
%
...
errSIRPAMean = sqrt(sum((nnS1ModelPA - nnS1).^2 + (nnI1ModelPA - nnI1).^2 + (nnR1ModelPA - nnR1).^2))/numel(nnI1);
errSIRPairBasedMean = sqrt(sum((nnS1ModelPairBased - nnS1).^2 + (nnI1ModelPairBased - nnI1).^2 + (nnR1ModelPairBased - nnR1).^2))/numel(nnI1);
errSIRREDMMFMean = sqrt(sum((nnS1ModelREDM - nnS1).^2 + (nnI1ModelREDM - nnI1).^2 + (nnR1ModelREDM - nnR1).^2))/numel(nnI1);

errSIRDimMean = sqrt(sum((nnI1ModelDim - nnI1).^2))/numel(nnI1); errSIRPAMean = sqrt(sum((nnI1ModelPA - nnI1).^2))/numel(nnI1); errSIRPairBasedMean = sqrt(sum((nnI1ModelPairBased' - nnI1).^2))/numel(nnI1); errSIRREDMMean = sqrt(sum((nnI1ModelREDM' - nnI1).^2))/numel(nnI1);

legCell = {'True', 'Hom. pair', 'Red. eff. deg.', 'Pair-based', 'Dim.'};

disp(['Mean RMS Error over t ∈ {0, 1, ..., ', num2str(max(tt1)), '} [Hom. pair, Red. eff. deg., Pair-based, Dim.]:']);
